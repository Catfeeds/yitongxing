...
<html lang="en">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="portlet light bordered">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <span class="caption-subject font-green bold uppercase">订单管理</span>
                                    </div>
                                            ...
                                            <tbody>
                                            	...
                                            	<c:forEach items="${list}" var="list">
	                                            	<tr>
	                                                    <td> ${list.orderid} </td>
	                                                    <td> 
	                                                    	<c:if test="${list.state==1}"><font>待接单</font></c:if>
	                                                    	<c:if test="${list.state==2}"><font>待确认</font></c:if>
	                                                    	<c:if test="${list.state==3}"><font>待付款</font></c:if>
	                                                    	<c:if test="${list.state==4}"><font>订单进行中</font></c:if>
	                                                    	<c:if test="${list.state==5}"><font>待确认订单完成</font></c:if>
	                                                    	<c:if test="${list.state==6}"><font>待评价</font></c:if>
	                                                    	<c:if test="${list.state==7}"><font color="green">已完成</font></c:if>
	                                                    	<c:if test="${list.state==8}"><font color="red">已取消</font></c:if>
	                                                    </td>
	                                                    ...
	                                                    <!-- <td>
	                                                        <span class="label label-sm label-info" onclick="check(${list.orderid});">查看</span>
	                                                    </td> -->
	                                                </tr>
												</c:forEach>	
												<!-- 遍历数据 end -->
	...
	<script type="text/javascript">
		...
		$(function(){
	        ...
	        $(".page-sidebar-menu").children("li[name='orderManagement']").children("li[name='order']").children("a").children("span[class='arrow']").addClass("open");
	        $(".page-sidebar-menu").children("li[name='orderManagement']").children("ul").children("li[name='order']").addClass("active");
